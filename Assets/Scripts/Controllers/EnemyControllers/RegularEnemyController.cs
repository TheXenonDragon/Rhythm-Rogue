...
public class RegularEnemyController : EnemyController
{
    ...
    public void StartEnemy(Vector2Int spawnPostion, BeatController beatController, MapManager mapManager, EnemySpawnManager spawnManager, GameObject player, float tileSize, float moveSpeed, int health, int damage){
        ...
        base.StartEnemy(spawnPostion, beatController, mapManager, player, UnitSize.Regular, tileSize, moveSpeed, health, damage);
    ...
    protected override void Wander(){
        if(waitingForNewPath && checkpointIndex >= checkpoints.Length){
            ...
            PathRequestManager.RequestPath (new PathRequest(currentPosition, spawnManager.GetNewDestination(), OnPathFound), true);
    ...
    protected override void Chase(){
        pathfindingGrid.CreateGrid(mapManager.GetMap());
        base.Chase();     
    }
    ...
    protected override void TestForPossibleAttack(){
        ...
        foreach (Vector2Int direction in directions){
            if (mapManager.PositionHasPlayer(currentPosition + direction))
            {
